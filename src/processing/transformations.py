...
GPS_SCHEMA = StructType(KAFKA_ENVELOPE_SCHEMA.fields + [
...
IMU_SCHEMA = StructType(KAFKA_ENVELOPE_SCHEMA.fields + [
...
CAN_BUS_SCHEMA = StructType(KAFKA_ENVELOPE_SCHEMA.fields + [
    StructField("speed_ms", DoubleType(), False),
    ...
    return (
        df.withColumn("speed_kmh", F.round(F.col("speed_ms") * 3.6, 2))
          ...
          .withColumn("processing_lag_ms",
              (F.unix_timestamp(F.current_timestamp()) - F.unix_timestamp("timestamp")) * 1000)
    )
...
def deduplicate(df: DataFrame, key_col: str = "event_id", watermark_col: str = "timestamp") -> DataFrame:
